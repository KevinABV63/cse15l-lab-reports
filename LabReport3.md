...
## The Symptom
### Failure Inducing
![Failure](Failure_Inducing.png)

### Non Failure
![Nonfailure](Non_Failure.png)

Symptom: The output array is not reversed; it remains the same as the input array.

Output: Original code would likely fail this test.
...
./technical/biomed
```
The `find -name` command finds the biomed folder located in the technical folder.
...
```
The `find -name` command finds all the files in the `/technical` folder ending with the ".txt" extension.
...
```
`find-size` looks for files that are 20 kilobytes in size.
### Example 4
```
...
./technical/plos
```
`find -size` looks for files that are smaller than 1 megabyte.
...
./technical/biomed/1471-2431-3-3.txt
```
`find -mtime` looks for all files modified less than 7 days ago.
### Example 6
```
...
./technical/biomed/1477-7827-1-17.txt
```
`find -atime` looks for files accessed 0 days ago.

## Example 8
```
$ find ./technical -atime +5

```
`find -atime` looks for files accessed more than 5 days ago. No files were accessed more than 5 days ago.
